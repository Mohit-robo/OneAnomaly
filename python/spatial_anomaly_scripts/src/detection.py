    ...
    with torch.inference_mode():
        ...
        for img_ref_n in tqdm(img_ref_samples, desc="Building memory bank", leave=False):
            ...
            for i in range(len(img_augmented)):
                ...
                mask_ref = model.compute_background_mask(features_ref_i, grid_size1, threshold=mask_threshold, masking_type=(mask_ref_images and masking))
        ...
        if faiss_on_cpu:
            ...
            try:
                ...
                print("Warning: faiss-gpu not found or StandardGpuResources missing. Falling back to CPU search.")
                knn_index = faiss.IndexFlatL2(features_ref.shape[1])
            except Exception as e:
                print(f"Warning: Failed to initialize FAISS GPU search: {e}. Falling back to CPU search.")
                knn_index = faiss.IndexFlatL2(features_ref.shape[1])
        ...
        for type_anomaly in tqdm(type_anomalies, desc = f"processing test samples ({object_name})"):
            ...
            for idx, img_test_nr in tqdm(enumerate(sorted(os.listdir(data_dir))), desc=f"Evaluating object_name'{type_anomaly}'", leave=False, total=len(os.listdir(data_dir))):
                ...
                if masking:
                    mask2 = model.compute_background_mask(features2, grid_size2, threshold=mask_threshold, masking_type=masking)
                else:
                ...
                if save_images:
                    save_dir_imgs = f"{plots_dir}/anomaly_maps/seed={seed}/{object_name}/test/{type_anomaly}/"
                    os.makedirs(save_dir_imgs, exist_ok=True)
                    anomaly_map = dists2map(d_masked, image_test.shape)
                    # Normalize and save as PNG heatmap
                    plt.imsave(f"{save_dir_imgs}/{img_test_nr}.png", anomaly_map, cmap='jet')
                ...
                if save_examples and idx < 3:
                    ...
                    plt.tight_layout()
                    plt.savefig(f"{example_dir}/example_{type_anomaly}_{idx}.png")
                    plt.close()
