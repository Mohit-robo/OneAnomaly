...
def parse_dataset_files(object_name, dataset_base_dir, anomaly_maps_dir, dataset="MVTec"):
    ...
    for subdir in listdir(str(test_dir)):
        ...
        test_images = []
        if path.exists(path.join(test_dir, subdir)):
            test_images = [path.splitext(file)[0]
                           for file
                           in listdir(path.join(test_dir, subdir))
                           if path.splitext(file)[1].lower() in ['.png', '.jpg', '.jpeg', '.tiff']]
        ...
        if subdir != 'good':
            for file in test_images:
                ...
                for ext in ['.png', '.jpg', '.jpeg']:
                    for suffix in ['_mask', '']:
                        gt_path = path.join(gt_base_dir, subdir, file + suffix + ext)
                        if path.exists(gt_path):
                            gt_filenames.append(gt_path)
                            gt_found = True
                            break
                    if gt_found: break
                if not gt_found:
                    gt_filenames.append(None)
        else:
...
def eval_segmentation(gt_filenames, prediction_filenames, pro_integration_limit=0.3, delete_tiff_files=True):
    ...
    # optionally delete all npy and tiff files in the anomaly_maps_dir to save disk space
    if delete_tiff_files:
        for pred_name in prediction_filenames:
            if path.exists(pred_name + '.tiff'):
                os.remove(pred_name + '.tiff')
            if path.exists(pred_name + '.npy'):
                os.remove(pred_name + '.npy')
...
def get_objects_from_dataset(dataset, dataset_base_dir=None):
    ...
    elif dataset_base_dir is not None:
        # Infer objects from the directory structure
        objects = [d for d in listdir(dataset_base_dir) if path.isdir(path.join(dataset_base_dir, d)) and path.exists(path.join(dataset_base_dir, d, 'test'))]
    else:
        objects = []
    return objects
    

def eval_finished_run(dataset, dataset_base_dir, anomaly_maps_dir, output_dir, seed = None, pro_integration_limit = 0.3, eval_clf = True, eval_segm = False, delete_tiff_files = True, aggregation_statistics = "meantop1p", obj_list=None):
    """
