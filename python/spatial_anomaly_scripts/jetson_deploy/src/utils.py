...
def augment_image(img_ref, augmentation = "rotate", angles = [0, 45, 90, 135, 180, 225, 270, 315]):
    ...
    if augmentation == "rotate":
        ...
        imgs.append(img_ref)
        imgs.append(cv2.flip(img_ref, 1))
    else:
        imgs.append(img_ref)
    return imgs
...
def dists2map(dists, img_shape, sigma=4):
    ...
    # Use OpenCV GaussianBlur instead of scipy for 2-3x speedup
    # Convert sigma to kernel size: ksize = 2 * int(4 * sigma + 0.5) + 1
    ksize = 2 * int(4 * sigma + 0.5) + 1
    dists = cv2.GaussianBlur(dists, (ksize, ksize), sigma)
...
def get_dataset_info(dataset, preprocess, data_path=None):
    ...
    if dataset == "MVTec":
        ...
        object_anomalies = {"bottle": ["damage"],
        ...
        for o in os.listdir(data_path):
            ...
            if os.path.isdir(anomalies_path) and os.path.exists(os.path.join(anomalies_path, "test")):
                objects.append(o)
        ...
        if len(objects) == 0:
            # Maybe the data_path IS the object folder?
            if os.path.exists(os.path.join(data_path, "test")) and os.path.exists(os.path.join(data_path, "train")):
                o = os.path.basename(data_path.rstrip("/"))
                objects.append(o)
                masking_default[o] = False
                rotation_default[o] = True
                anomaly_types = []
                test_path = os.path.join(data_path, "test")
                for anomalytype in os.listdir(test_path):
                    if anomalytype != "good" and os.path.isdir(os.path.join(test_path, anomalytype)):
                        anomaly_types.append(anomalytype)
                object_anomalies[o] = anomaly_types
                # Adjust data_path to parent so that os.path.join(data_path, object) works later
                print(f"Detected single object structure. Using '{o}' as object name.")
            else:
                raise ValueError(f"No valid objects found in {data_path}. Ensure it follows MVTec structure (data_path/object/test/...) or point directly to an object folder (data_path/test/...).")

        print(f"Found {len(objects)} objects: {objects}")
        if len(objects) > 0:
            print(f"Average {np.mean([len(object_anomalies[o]) for o in objects]):.1f} anomaly types per object.")
