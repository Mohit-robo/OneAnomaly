...
from src.backbones_trt import DINOv3TensorRTWrapper
...
def parse_args():
    ...
    parser.add_argument("--trt_path", type=str, required=True, help="Path to TensorRT engine file.")
    parser.add_argument("--output_dir", type=str, default="inference_results_trt", help="Where to save visual results.")
...
def main():
    ...
    print(f"Loading TensorRT engine: {args.trt_path}...")
    ...
    for img_path in tqdm(all_test_images, desc="Detecting anomalies"):
        ...
        axes[0].set_title("Original Image (TensorRT)")
        axes[0].axis('off')
